...
if(!IsPC()){
	location.href = "/pages/m.release.html";
}

define(function(){
	return  function(templateUrl,param){
		$("#container").loadPage(templateUrl,["css/release.css"],function(){

			$("#logo").find("a").attr("href","javascript:")

			
			//地图
			$("#relese_map").loadPage(templateUrl,["css/contactUs.css"],function(){
			...
			Conference.getServeTime("",function(data){
				...
				if(data.code == 200){
					...
					function countdown(){
						...
						setTimeout(countdown,1000); //循环倒计时
			...
			function getAuthCode(){
				...
				//提交验证
				if(params.authCode.phone  == ""){$.alertTmpMsg("请填写您的电话号码"); return}
